...
function randomPostId() {
  ...
  return Array.from(crypto.getRandomValues(new Uint8Array(11)), (b) => abc[b & 63]).join("");
}
...
async function runTest(e) {
  ...
  const post = { id, text: $("testTitle").value.trim(), author: $("testChannel").value.trim(), headline: "" };
  ...
  verdict.lastChild.textContent = `${Math.round(r.confidence * 100)}% sure${r.bait >= 0.75 ? " · ⚡ engagement bait" : ""}`;
