...
async function refresh() {
  ...
  else if (!stats) el.textContent = "Open your LinkedIn feed to see live counts.";
...
$("recheck").onclick = async () => {
  ...
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  // Only reload tabs where LinkedOut is running (i.e. LinkedIn).
  const onLinkedIn = tab && (await chrome.tabs.sendMessage(tab.id, { type: "pageStats" }).catch(() => null));
  if (onLinkedIn) chrome.tabs.reload(tab.id);
  window.close();
};
